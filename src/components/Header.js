...
function Header() {
  ...
  const navigationItems = [
    { id: "home", label: "Home", action: () => navigate("/"), showOnlyOutsideHome: true },
    { id: "aboutUs", label: "About Us", action: () => handleNavigation("aboutUs") },
    { id: "services", label: "Services", action: () => handleNavigation("services") },
    { id: "contact", label: "Contact", action: () => handleNavigation("contactUs") },
  ];
  ...
  return (
    <div
      className={`fixed top-0 left-0 right-0 z-50 h-20 sm:h-16 bg-zero mt-5 sm:mt-3 mx-3
        ...
        ${isMenuOpen ? "rounded-t-3xl rounded-b-none" : "rounded-3xl"}
      `}
    >
      ...
      <div
        className={`lg:hidden bg-zero overflow-hidden origin-top transition-all duration-500 ease-in-out
          ${isMenuOpen ? "transform max-h-screen opacity-100 rounded-b-3xl" : "transform max-h-0"}
        `}
      >
        <nav className="px-7">
